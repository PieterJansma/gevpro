...
class TestPets:
    def test_cat_sound(self):
        """Tests if cat.sound() is empty, if its empty it displays an error"""
        cat = pets.Cat('Lord Purr')
        assert cat.sound(), 'No sound given'

    def test_dog_sound(self):
        """Tests if dog.sound() is empty, if its empty it displays an error"""
        dog = pets.Dog('Peaches')
        assert dog.sound(), 'No sound given'